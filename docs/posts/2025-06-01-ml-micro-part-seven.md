...
# This script starts the MLflow server on localhost:8080
...
Go ahead execute the script to start MLFlow and check out the server UI, there won't be a whole lot here yet! Next we are going to be modifying our notebook to utilize MLFlow as well, since MLFlow will be a modeling dependency as well, we can add it by running `uv add mlflow` in the `housing-price-model` directory as well (importantly the same version of MLFlow will be used).
