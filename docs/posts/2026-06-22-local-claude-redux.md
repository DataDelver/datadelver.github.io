...
1. Quantization Method (`K`) — The method used to produce the quantization. `K` is the typical modern standard, though other older variants such as `0` exist as well.

1. Granularity (`M`) — Particularly with `K` Quants, you will see a suffix indicating its size (`S`, `M`, `L`), indicating the level of compression within that bit-depth.
